...
```mermaid
graph TD
    User((The Partner)) --> Core{PointLinker Engine}
    
    subgraph "Digital Presence"
        Core --> Web[100/100 Performance Web]
        Core --> Booking[Custom Scheduling]
        Core --> CRM[Customer Hub]
    end

    subgraph "Physical fulfillment"
        Core --> Ship[Automated Shipping Bridge]
        Ship --> Factory[Kunaki / Printful]
    end

    style Core fill:#4f46e5,stroke:#333,stroke-width:2px,color:#fff
```
